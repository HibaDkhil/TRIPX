...
public class TransportUserInterfaceController {
    ...
    @FXML private HBox userDisplayContainer;
    ...
    private static final String BTN_TEAL =
            "-fx-background-color: linear-gradient(to right, #4CCCAD, #7BE7D6); -fx-text-fill: white; -fx-font-weight: bold; " +
                    "-fx-padding: 9 22 9 22; -fx-cursor: hand; -fx-background-radius: 10; -fx-effect: dropshadow(gaussian, rgba(76,204,173,0.18), 10, 0, 0, 2);";
    private static final String BTN_DARK =
            "-fx-background-color: linear-gradient(to right, #6D83F2, #8B9CFF); -fx-text-fill: white; -fx-font-weight: bold; " +
                    "-fx-padding: 9 22 9 22; -fx-cursor: hand; -fx-background-radius: 10; -fx-effect: dropshadow(gaussian, rgba(99,102,241,0.16), 10, 0, 0, 2);";
    ...
    private static final String BTN_DETAIL =
            "-fx-background-color: rgba(255,255,255,0.92); -fx-text-fill: #0A153A; -fx-font-weight: 900; " +
    ...
    private static final String TOGGLE_ON =
            "-fx-background-color: linear-gradient(to right, #6D83F2, #4CCCAD); -fx-text-fill: white; -fx-font-weight: bold; " +
                    "-fx-padding: 9 26 9 26; -fx-cursor: hand; -fx-font-size: 13px; -fx-background-radius: 12;";
    private static final String TOGGLE_OFF =
            "-fx-background-color: rgba(2,6,23,0.06); -fx-text-fill: #0A153A; " +
                    "-fx-padding: 9 26 9 26; -fx-cursor: hand; -fx-font-size: 13px; -fx-background-radius: 12;";
    ...
    public void setCurrentUser(User user) {
        ...
        // Track transport page visit
        if (currentUser != null) {
            tn.esprit.utils.ActivityLogger.logVisit(currentUser, "TRANSPORT");
        }
    }
    ...
    /**
     * Navigate to another FXML view while preserving user session
     */
    private void navigateTo(String fxmlPath) {
        try {
            ...
            // Pass current user to the new controller
            if (currentUser != null) {
                if (controller instanceof HomeController) {
                    ((HomeController) controller).setUser(currentUser);
                } else if (controller instanceof UserDestinationsController) {
                    ((UserDestinationsController) controller).setCurrentUser(currentUser);
                } else if (controller instanceof UserActivitiesController) {
                    ((UserActivitiesController) controller).setCurrentUser(currentUser);
                } else if (controller instanceof AccommodationsController) {
                    ((AccommodationsController) controller).setCurrentUser(currentUser);
                } else if (controller instanceof UserBookingsController) {
            ...
            e.printStackTrace();
            showAlert("Could not navigate: " + e.getMessage());
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private Map<Long, String> getDestinationName() {
        ...
        try {
            ...
            String sql = "SELECT destination_id AS id, name FROM destination_trans ORDER BY name";
    ...
    private void buildSchedulesView() {
        ...
        searchSwap.setVisible(false);
        searchSwap.setManaged(false);
        ...
        tg.selectedToggleProperty().addListener((obs, old, nv) -> {
            ...
            if (nv == allBtn) {
                ...
                searchSwap.setVisible(true);
                searchSwap.setManaged(true);
    ...
    private VBox buildFlightSearchPanel(List<Schedule> all, Map<Integer, Transport> tMap,
        ...
        fromField.setStyle(FORM_FIELD);
        fromField.setPrefWidth(210);
        ...
        clsBox.setValue("Any Class");
        clsBox.setStyle(FORM_FIELD);
        clsBox.setMaxWidth(Double.MAX_VALUE);
        ...
        hdr.setStyle("-fx-font-weight:900;-fx-text-fill:#0A153A;-fx-font-size:13px;");

        GridPane g = buildFormGrid(158, 215);
        int r = 0;
        g.addRow(r++, fl("From (airport/city)"), fromField);
        g.addRow(r++, fl("To (arrival city)"), toBox);
        g.addRow(r++, fl("Departure date"), depDp);
        g.addRow(r++, fl("Travel class"), clsBox);

        HBox btnRow = new HBox(10, searchBtn, clearBtn);
    ...
    private VBox buildVehicleSearchPanel(List<Schedule> all, Map<Integer, Transport> tMap,
        ...
        searchBtn.setOnAction(e -> {
            ...
            LocalDate wStart = startDp.getValue();
            LocalDate wEnd = endDp.getValue();
        ...
        clearBtn.setOnAction(e -> {
            placeBox.setValue("Any Location");
    ...
    private VBox buildScheduleCard(Schedule s, Transport t,
        ...
        String hdrColor = unavail ? "#94A3B8" : (flight ? "#6D83F2" : "#4CCCAD");
        ...
        routeLbl.setStyle("-fx-text-fill:#444;-fx-font-size:12px;");
        routeLbl.setWrapText(true);
        ...
        priceLbl.setStyle("-fx-text-fill:#0A153A;-fx-font-weight:900;-fx-font-size:14px;");
    ...
    private void openTransportTypePopup() {
        ...
        hdrBar.setStyle("-fx-background-color:linear-gradient(to right,#6D83F2,#4CCCAD);-fx-padding:18 24 18 24;");
        hdrBar.setAlignment(Pos.CENTER_LEFT);

        VBox fc = typeCard("Flight", "Search & book scheduled\nflights worldwide", "#6D83F2");
        VBox vc = typeCard("Vehicle Rental", "Rent a car or vehicle\nfor your trip", "#4CCCAD");
        fc.setOnMouseClicked(e -> {
            popup.close();
            buildProviderList("FLIGHT");
        });
        vc.setOnMouseClicked(e -> {
            popup.close();
            buildProviderList("VEHICLE");
        });

        HBox body = new HBox(24, fc, vc);
        body.setAlignment(Pos.CENTER);
        body.setPadding(new Insets(30, 36, 30, 36));
    ...
    private VBox typeCard(String title, String sub, String color) {
        ...
        sl.setWrapText(true);
        sl.setTextAlignment(javafx.scene.text.TextAlignment.CENTER);
        VBox card = new VBox(10, tl, sl);
        card.setAlignment(Pos.CENTER);
        card.setPrefSize(182, 130);
    ...
    private void buildProviderList(String type) {
        ...
        fp.setHgap(20);
        fp.setVgap(20);
        fp.setPadding(new Insets(12, 0, 12, 0));

        String[] palette = {"#6D83F2", "#4CCCAD", "#8B9CFF", "#FFB4D9", "#FFD6A5",
                "#27ae60", "#7BE7D6", "#94A3B8", "#7B68EE", "#16a085"};
        int[] idx = {0};

        for (Map.Entry<String, Long> en : counts.entrySet()) {
            String prov = en.getKey();
            ...
            pc.setPrefSize(200, 110);
            pc.setPadding(new Insets(18));
        ...
        ScrollPane sc = scrollPane(fp);
        VBox.setVgrow(sc, Priority.ALWAYS);
        VBox layout = new VBox(16, topBar, sc);
        VBox.setVgrow(sc, Priority.ALWAYS);
        contentArea.getChildren().setAll(layout);
    }
    ...
    private void buildTransportCards(String type, String provider) {
        ...
        ScrollPane sc = scrollPane(fp);
        VBox.setVgrow(sc, Priority.ALWAYS);
        VBox layout = new VBox(14, topBar, sc);
        VBox.setVgrow(sc, Priority.ALWAYS);
        contentArea.getChildren().setAll(layout);
    }
    ...
    private VBox buildTransportCard(Transport t) {
        ...
        pr.setStyle("-fx-text-fill:#0A153A;-fx-font-weight:900;-fx-font-size:14px;");
        ...
        am.setStyle("-fx-text-fill:#666;-fx-font-size:11px;");
        am.setWrapText(true);
        am.setMaxWidth(240);

        VBox body = new VBox(8, badges, pr, cp, am);
        body.setPadding(new Insets(12, 16, 12, 16));

        Button det = new Button("View Details");
        det.setStyle(BTN_DETAIL);
        det.setMaxWidth(Double.MAX_VALUE);
        det.setOnAction(e -> showTransportDetail(t));

        Button bk = new Button("Book Direct");
        bk.setStyle(BTN_BOOK);
        bk.setMaxWidth(Double.MAX_VALUE);
        bk.setOnAction(e -> openBookingForm(t, null, null));

        HBox btns = new HBox(det, bk);
        HBox.setHgrow(det, Priority.ALWAYS);
        HBox.setHgrow(bk, Priority.ALWAYS);
    ...
    private void showTransportDetail(Transport t) {
        ...
        st.setStyle("-fx-font-weight:900;-fx-text-fill:#0A153A;-fx-font-size:13px;-fx-padding:6 0 4 0;");
        ...
        for (Schedule s : scheds) {
            ...
            il.setStyle("-fx-text-fill:#333;-fx-font-size:11px;");
            il.setWrapText(true);
            ...
            bb.setOnAction(ev -> {
                popup.close();
                openBookingForm(t, fs, null);
            });

            HBox row = new HBox(10, il, bb);
            row.setAlignment(Pos.CENTER_LEFT);
            row.setStyle("-fx-background-color:#F8FAFC;-fx-padding:8 12 8 12;-fx-background-radius:5;");
    ...
    private void buildMyBookingsView() {
        ...
        List<Bookingtrans> bks = bookingService.getBookingsByUserId(currentUser.getUserId());
        Map<Integer, Transport> tMap = new HashMap<>();
        ...
        sum.setStyle("-fx-text-fill:#0A153A;-fx-font-weight:900;-fx-background-color:rgba(255,255,255,0.92);" +
        ...
        ScrollPane sc = scrollPane(fp);
        VBox.setVgrow(sc, Priority.ALWAYS);
        VBox layout = new VBox(14, topBar, sum, sc);
        VBox.setVgrow(sc, Priority.ALWAYS);
        contentArea.getChildren().setAll(layout);
    }
    ...
    private VBox buildBookingCard(Bookingtrans b, Transport t) {
        ...
        String typeBadgeColor = "FLIGHT".equals(typeText) ? "#6D83F2" : "#4CCCAD";
        HBox badges = new HBox(6,
        ...
        pl.setStyle("-fx-text-fill:#0A153A;-fx-font-weight:900;-fx-font-size:13px;");
        ...
        if (direct && !cancelled) {
            ...
            act.setOnAction(e -> {
                if (confirm("Cancel booking #" + b.getBookingId() + "?")) {
                    ...
                    b.setCancellationReason("Cancelled by user");
        ...
        HBox.setHgrow(det, Priority.ALWAYS);
        HBox.setHgrow(act, Priority.ALWAYS);
    ...
    private void viewBookingDetail(Bookingtrans b) {
        ...
        popup.initModality(Modality.APPLICATION_MODAL);
        popup.initOwner(contentArea.getScene().getWindow());
        popup.setTitle("Booking #" + b.getBookingId());
        popup.setResizable(false);

        Label h = new Label("Booking #" + b.getBookingId() + "  Details");
        h.setStyle("-fx-font-size:15px;-fx-font-weight:bold;-fx-text-fill:white;");
        HBox hb = new HBox(h);
        hb.setStyle("-fx-background-color:linear-gradient(to right,#6D83F2,#4CCCAD);-fx-padding:16 22 16 22;");
        ...
        GridPane g = buildFormGrid(160, 270);
        int r = 0;
    ...
    private void openScheduleSelectForBooking(Bookingtrans b, Transport t) {
        if (t == null) {
            showAlert("Transport details unavailable.");
            return;
        }
        Stage popup = new Stage();
        popup.initModality(Modality.APPLICATION_MODAL);
        popup.initOwner(contentArea.getScene().getWindow());
        popup.setTitle("Add Schedule to Booking #" + b.getBookingId());
        popup.setResizable(false);
        ...
        if (eligible.isEmpty()) {
            ...
            for (Schedule s : eligible) {
                ...
                Label il = new Label(info);
                il.setStyle("-fx-text-fill:#333;-fx-font-size:12px;");
                il.setWrapText(true);
                HBox.setHgrow(il, Priority.ALWAYS);
        ...
        Button cb = mkBtn("Cancel", BTN_DARK);
        cb.setOnAction(e -> popup.close());
        HBox br = new HBox(cb);
        br.setAlignment(Pos.CENTER_RIGHT);
        br.setPadding(new Insets(10, 0, 0, 0));
    ...
    private void openBookingForm(Transport transport, Schedule schedule, Stage ownerStage) {
        ...
        if (schedule != null) {
            ...
            bl.setStyle("-fx-text-fill:#0A153A;-fx-font-size:12px;-fx-background-color:rgba(76,204,173,0.14);" +
                    "-fx-padding:10 14 10 14;-fx-background-radius:5;");
            bl.setWrapText(true);
            banner.getChildren().add(bl);
        ...
        adSpin.setEditable(true);
        adSpin.setMaxWidth(Double.MAX_VALUE);

        Spinner<Integer> chSpin = new Spinner<>(0, 20, 0);
        chSpin.setEditable(true);
        chSpin.setMaxWidth(Double.MAX_VALUE);

        CheckBox ins = new CheckBox("Add travel insurance  (+25 EUR per seat)");
        ins.setStyle("-fx-text-fill:#0A153A;");

        Label prev = new Label();
        prev.setWrapText(true);
        prev.setStyle("-fx-text-fill:#0A153A;-fx-font-weight:900;-fx-font-size:13px;" +
                "-fx-background-color:rgba(255,255,255,0.92);-fx-padding:10 14 10 14;-fx-background-radius:10;" +
                "-fx-effect:dropshadow(gaussian,rgba(2,6,23,.06),10,0,0,2);");
        ...
        GridPane g = buildFormGrid(165, 270);
        int r = 0;
        g.addRow(r++, fl("Travel Class"), clsBox);
        ...
        Button canc = mkBtn("Cancel", BTN_DARK);
        ...
        conf.setOnAction(e -> {
            try {
                ...
                bk.setUserId(currentUser.getUserId());
    ...
    private Label fl(String t) {
        Label l = new Label(t + ":");
        l.setStyle("-fx-font-weight:800;-fx-text-fill:#0A153A;-fx-font-size:12px;");
    ...
    private Label sectionTitle(String t) {
        Label l = new Label(t);
        l.setStyle("-fx-font-size:17px;-fx-font-weight:900;-fx-text-fill:#0A153A;");
        return l;
    }
    ...
    private Label errLabel() {
        ...
        l.setWrapText(true);
        l.setMaxWidth(430);
        l.setStyle("-fx-text-fill:#c0392b;-fx-font-size:11px;-fx-font-weight:bold;");
        return l;
    }

    private Button mkBtn(String t, String s) {
    ...
    private Long parseDestId(String val, Map<Long, String> dn) {
        ...
        try {
            return Long.parseLong(val.replace("Destination #", "").trim());
    ...
    private void showTransportOptimalRoutePopup(
        ...
        title.setStyle("-fx-font-size:16px;-fx-font-weight:900;-fx-text-fill:#0A153A;");
